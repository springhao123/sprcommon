...
import com.spr.game.common.client.SCConnection;
...
@Log4j2
@Component
public class SSNetServer implements Runnable {
	/**
	 * 服务器连接通道
	 */
	 ...
	 * 所有的连接数组
	 */
	protected ArrayList<SSConnection> connectionList;
	...
	public SSNetServer(int bufferSize, int clientSize) {
		data = new byte[bufferSize];
		nioWritedelay = new NioWriteDelay(10, 10);
		buffer = ByteBuffer.wrap(data, 0, data.length);
		connectionList = new ArrayList<>(clientSize);
	}
			...
			if (selectionkey == serverKey) {
				if (selectionkey.isAcceptable()) {
					try {
						...
						// 新建一个连接对象
						SSConnection nioconnection2 = new SSConnection(socketchannel);
				...
				SCConnection nioconnection = (SCConnection) selectionkey.attachment();
				if (nioconnection != null) {
					log.debug("client close the connect " + nioconnection.toString());
					nioconnection.close();
				}
			} else if (selectionkey.isReadable()) {
				// 有数据读
				SSConnection nioconnection1 = (SSConnection) selectionkey.attachment();
				try {
					...
					log.debug("[Unexpected close] closed while reading data:" + nioconnection1, ioexception1);
